...
import { api, ApiError, demoFetch } from "./api";
...
function Login({ onLogin }: { config: Config; onLogin: (u: User) => void }) {
  const [error, setError] = useState("");
  return <main className="loading"><Brand /><h1>Choose a demo persona.</h1>
    <p>Fictional people. No account or password needed. Changes stay in this tab.</p>
    <div className="demo-personas">{["alice", "bob", "admin"].map(username => <button className="primary" key={username}
      onClick={() => api<User>("/session", {method:"POST", body:JSON.stringify({username})}).then(onLogin).catch(e=>setError(message(e)))}>{username}</button>)}</div>
    {error && <p role="alert">{error}</p>}<p><a href="https://github.com/DimaGutierrez/slotguard">Explore the full backend on GitHub</a></p></main>;
}
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="workspace-label">
          ...
          <div>
            Studio workspace
            <small>{config.demo ? "Browser simulation" : "Single workspace"}</small>
          </div>
        </div>
        ...
        <div className="sidebar-note">
          <ShieldCheck size={23} />
          <strong>Explore the booking flow.</strong>
          <p>
            This public demo simulates conflicts in browser memory. The full backend uses PostgreSQL.
          </p>
          {config.demo && (
        ...
        <div className="profile">
          ...
          <button aria-label="Switch demo persona" title="Switch demo persona" onClick={logout}>
            <LogOut size={17} />
          </button>
        </div>
      </aside>
      <main className="main">
        <header className="topbar">
          ...
          <span className="connection">
            <i />
            {config.demo ? "SIMULATED DEMO" : "Local workspace"}
          </span>
        </header>
        <div className="page-content">
          <div className="public-demo-note"><strong>Interactive browser demo</strong> · Try booking a room, resolving a conflict or running the simulated race. No real reservations are made. Changes reset on reload.
            <div><a href="https://github.com/DimaGutierrez/slotguard">Full backend on GitHub ↗</a><a href="/">← Portfolio</a><button onClick={()=>location.reload()}>Reset demo</button></div>
          </div>
          {error && (
          ...
          {tab === "agenda" && (
            <>
              ...
              <div className="bottom-caption">
                <span>
                  <ShieldCheck size={14} />
                  Demo reservations exist only in this browser tab. Reload to reset.
                </span>
    ...
    try {
      ...
      const results = await Promise.all(
        s.tokens.map(async (token, index) => {
          ...
          const response = await demoFetch(`/api/demo/scenarios/${s.id}/book`, {
            method: "POST",
  ...
  return (
    <section className="lab">
      ...
      <div className="lab-intro">
        <span className="pill">SIMULATED REQUESTS · NO DATABASE</span>
        <p>
          Explore the outcome of two requests for one interval. Artificial delays choose the simulated winner; no HTTP request or database transaction occurs.
        </p>
      </div>
        ...
        {[0, 1].map((i) => {
          ...
          return (
            ...
            >
              ...
              {a && (
                <code>
                  Simulated {a.code} · {a.ms} ms (artificial delay)
                </code>
              )}
            </div>
          );
      ...
      <div className="race-actions">
        ...
        <span>
          Each run uses an isolated simulation. Your sample agenda stays separate.
        </span>
      </div>
      {confirmed !== undefined && (
        <div role="status" className={passed ? "race-result" : "error"}>
          ...
          <div>
            <strong>
              {passed
                ? "One simulated booking. Stored in browser memory."
                : "Unexpected result — inspect the responses."}
            </strong>
            <p>
              {confirmed} simulated booking{confirmed === 1 ? "" : "s"} ·{" "}
              {attempts.map((a) => a.code).join(" + ")} · Artificially varied winner.
            </p>
          </div>
        </div>
      )}
      <div className="explain-grid">
        ...
        <article>
          <span>02</span>
          <h3>The backend uses PostgreSQL.</h3>
          <p>
            In the full backend, an exclusion constraint protects each room. This browser demo illustrates the outcome only.
          </p>
        </article>
        <article>
          <span>03</span>
      ...
      <p className="footnote">
        Timings include artificial delays. This simulation demonstrates the interface, not database correctness or performance. Run the full backend to test real concurrency. Reloading clears this demo.
      </p>
    </section>
  );
}
